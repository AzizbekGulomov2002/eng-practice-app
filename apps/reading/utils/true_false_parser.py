...
class TrueFalseNotGivenParser:
    ...
    def parse_and_insert_inputs(self, html_content):
        ...
        try:
            ...
            # CRITICAL: Check if all questions already have question-input tags
            # Extract question range from HTML
            range_match = re.search(r'Questions?\s+(\d+)[-–—](\d+)', cleaned_html, re.IGNORECASE)
            if range_match:
                start_q = int(range_match.group(1))
                end_q = int(range_match.group(2))
                expected_questions = set(range(start_q, end_q + 1))
                
                # Find all existing question-input tags in this range
                existing_tags = set()
                existing_pattern = r'<question-input[^>]*data-question-number="(\d+)"[^>]*>'
                for match in re.finditer(existing_pattern, cleaned_html):
                    q_num = int(match.group(1))
                    if start_q <= q_num <= end_q:
                        existing_tags.add(q_num)
                
                # If all questions already have tags, preserve the section as-is
                if existing_tags.issuperset(expected_questions) or existing_tags == expected_questions:
                    print(f"[TFNG] ✅ All questions {start_q}-{end_q} already have question-input tags, preserving as-is")
                    return html_content
