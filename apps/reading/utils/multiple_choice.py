...
class MultipleChoiceParser:
    ...
    def remove_duplicate_question_inputs(self, html_content, start_q, end_q):
        """AGGRESSIVE removal of ALL duplicate question-input tags"""
        try:
            self.log(f"🧹 AGGRESSIVE duplicate removal for Q{start_q}-Q{end_q}...")
            
            # Find all question-input tags
            pattern = rf'<question-input[^>]*data-question-number=["\'](\d+)["\'][^>]*></question-input>'
            matches = list(re.finditer(pattern, html_content, re.IGNORECASE))
            
            # Group by question number and keep only FIRST occurrence
            seen_questions = set()
            ...
            for match in reversed(matches):
                try:
                    q_num = int(re.search(r'data-question-number=["\'](\d+)["\']', match.group(0)).group(1))
                    if start_q <= q_num <= end_q:
                        if q_num in seen_questions:
                            # This is a duplicate - REMOVE IT
                            result_html = result_html[:match.start()] + result_html[match.end():]
                            removed_count += 1
                            self.log(f"  🗑️ Removed duplicate Q{q_num} tag")
                        else:
            ...
            self.log(f"✅ Removed {removed_count} duplicate question-input tags")
            return result_html
    ...
    def detect_single_vs_multiple(self, html_content, plain_text):
        """Detect if this is single answer or multiple answer type - SIMPLIFIED"""
    ...
    def extract_all_questions_and_options_fixed(self, html_content, start_q, end_q):
        ...
        # This fixes the issue where questions share wrong options
        ...
        for p in soup.find_all('p'):
            ...
            for q_num in range(start_q, end_q + 1):
                if re.search(rf'\b{q_num}\b', p_text) or re.search(rf'<strong>\s*{q_num}\s*</strong>', p_html, re.IGNORECASE):
        ...
        if paragraphs_with_questions:
            for p, p_html, p_text in paragraphs_with_questions:
                if re.search(r'<br\s*/?>', p_html, re.IGNORECASE):
                    self.log(f"📋 Processing paragraph with <br /> format: {p_text[:100]}...")
                    ...
                    for line in lines:
                        ...
                        q_match = re.search(r'(\d+)(?:\s|&nbsp;){2,}(.+?)(?:<br|</p>|$)', line, re.IGNORECASE | re.DOTALL)
                        if not q_match:
                            q_match = re.search(r'(\d+)\s*(?:&nbsp;)*\s*(.+?)(?:<br|</p>|$)', line, re.IGNORECASE | re.DOTALL)
                        ...
                        if q_match:
                            ...
                            if start_q <= q_num <= end_q:
                                ...
                                # Skip if looks like option
                                if question_text and question_text[0].upper() in ['A', 'B', 'C'] and len(question_text) < 50:
                                    if not any(word in question_text.lower() for word in ['how', 'what', 'which', 'where', 'when', 'why', 'who', 'does', 'should', 'are']):
                                        continue
                        ...
                        opt_match = re.search(r'(?:&nbsp;|\s)+([A-Z])(?:\s|&nbsp;){2,}(.+?)(?:<br|</p>|$)', line, re.IGNORECASE)
                        if not opt_match:
                            opt_match = re.search(r'([A-Z])(?:\s|&nbsp;){2,}(.+?)(?:<br|</p>|$)', line, re.IGNORECASE)
                        if not opt_match:
                            opt_match = re.search(r'([A-Z])\s*(?:&nbsp;)*\s*(.+?)(?:<br|</p>|$)', line, re.IGNORECASE)
            ...
            if questions_data:
                self.log(f"📊 Successfully extracted {len(questions_data)} questions from <br /> format")
        ...
        for i, p in enumerate(p_tags):
            ...
            if q_num is not None and start_q <= q_num <= end_q:
                # SIMPLE: Always finalize previous question before starting new one
                if current_question_num is not None and current_question_num in questions_data:
                    self.log(f"✅ Finalized Q{current_question_num} with {len(questions_data[current_question_num]['options'])} options")
                ...
                # Create new question with empty options
                questions_data[q_num] = {
                    'text': question_text,
                    'options': [],
                    'p_index': i,
                ...
                current_question_num = q_num
                current_question = questions_data[q_num]
                self.log(f"✅ Created Q{q_num}: {question_text[:50]}...")
                continue
        ...
        # Check if we have any questions in the range
        questions_in_range = [q for q in questions_data.keys() if start_q <= q <= end_q]
        
        if len(questions_in_range) < (end_q - start_q + 1) and start_q <= end_q:
            self.log(f"🔍 CRITICAL: Missing questions in range {start_q}-{end_q}, checking for options-only format...")
            self.log(f"  📊 Found {len(questions_in_range)} questions, need {end_q - start_q + 1}")
            
            # Find all option paragraphs in the HTML
            options_found = []
            for i, p in enumerate(p_tags):
                p_text = p.get_text(strip=True)
                if not p_text or p_text in ['', ' ', '\xa0', '&nbsp;']:
                ...
                # Check if this is an option paragraph (starts with letter A-Z)
                opt_match = re.match(r'^([A-Z])\s+(.+)$', p_text)
                if not opt_match:
                    opt_match = re.match(r'^([A-Z])(?:\s|&nbsp;)+(.+)$', p_text)
            ...
            self.log(f"  📋 Found {len(options_found)} option paragraphs")
            
            # If we found options, create questions for the missing ones
            if options_found and len(options_found) >= len(allowed_letters):
                num_questions = end_q - start_q + 1
                options_per_question = len(allowed_letters)
                total_options_needed = num_questions * options_per_question
                
                self.log(f"  📊 Need {num_questions} questions, {options_per_question} options each, total {total_options_needed} options")
                
                # Strategy 1: If we have enough options for all questions (each question has its own set)
                if len(options_found) >= total_options_needed:
                    self.log(f"  ✅ Strategy 1: Enough options for all questions ({len(options_found)} >= {total_options_needed})")
                    for q_idx, q_num in enumerate(range(start_q, end_q + 1)):
                        if q_num not in questions_data:
                            ...
                            question_options = options_found[start_idx:end_idx]
                            
                            if len(question_options) == options_per_question:
                                questions_data[q_num] = {
                                    'text': f"Question {q_num}",  # Placeholder
                                    'options': question_options,
                                    'p_index': question_options[0]['index'],
                                    'original_html': question_options[0]['html']
                                }
                                self.log(f"  ✅ Created Q{q_num} with {len(question_options)} options")
                
                # Strategy 2: If we have at least one set of options, use them for all missing questions
                elif len(options_found) >= options_per_question:
                    self.log(f"  ✅ Strategy 2: Using shared options for all questions ({len(options_found)} options found)")
                    question_options = options_found[:options_per_question]
                    for q_num in range(start_q, end_q + 1):
                        if q_num not in questions_data:
                            questions_data[q_num] = {
                                'text': f"Question {q_num}",  # Placeholder
                                'options': question_options.copy(),
                                'p_index': question_options[0]['index'],
                                'original_html': question_options[0]['html']
                            }
                            self.log(f"  ✅ Created Q{q_num} with shared {len(question_options)} options")
    ...
    def build_perfect_format(self, questions_data, question_type, original_html=None):
        ...
        if original_html:
            ...
            # CRITICAL: Reset questions_processed to track all questions
            self.questions_processed = set()
            ...
            for q_num in sorted(questions_data.keys()):
                ...
                # Check if question-input tag already exists - if so, preserve it completely
                ...
                if existing_matches:
                    self.log(f"  ✅ Q{q_num} already has {len(existing_matches)} question-input tag(s) with proper format, preserving as-is")
                    # Check if the existing tag has data-options (proper format)
                    first_match = existing_matches[0]
                    existing_tag = result_html[first_match.start():first_match.end()]
                    if 'data-options' in existing_tag:
                        self.log(f"  ✅ Q{q_num} has data-options, preserving completely")
                        ...
                        self.log(f"  ⚠️ Q{q_num} has question-input tag but missing data-question-options, will update")
                        # Continue to update the tag with options
        ...
        for q_num in sorted(questions_data.keys()):
            ...
            question_text_clean = question_data["text"].strip()
            question_tag = f'<question-input data-question-number="{q_num}" data-question-type="multiple_choice" data-options=\'{options_json}\'>{question_text_clean}</question-input>'
